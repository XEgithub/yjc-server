...
import com.dk.data.dto.AddClassifyHomeDto;
import com.dk.data.dto.EditClassifyHomeDto;
 ...
 */
@Controller
@RequestMapping(value = "/admin/classifyhome")
public class ClassifyHomeController {

    private static final Logger logger = LoggerFactory.getLogger(ClassifyHomeController.class);

    @Autowired
    private ClassifyHomeService service;

    @GetMapping("v_list")
    String vlist(Model model){
        return "/admin/classifyhome/list";
    }

    @PostMapping("list")
    @ResponseBody
    PageVo<ClassifyHome> list(@RequestBody SearchClassifyHomeDto condition){
        logger.info("搜索轮播图，参数：\n {}", condition.toString());
        IPage<ClassifyHome> page = service.findAll(condition);
        return Result.page(page);
    }

    @GetMapping("v_add")
    String add(){
        return "/admin/classifyhome/add";
    }

    @PostMapping("add")
    @ResponseBody
    Vo add(@RequestBody @Validated AddClassifyHomeDto param, BindingResult result){
        ...
        ClassifyHome bean = new ClassifyHome();
        BeanUtil.copyProperties(param, bean);
        ClassifyHome repo = service.add(bean);
        return Result.ok();
    }
    ...
    String update(@PathVariable("id") Long id, Model model){
        ...
        return "/admin/classifyhome/edit";
    }

    @PostMapping("update")
    @ResponseBody
    Vo update(@RequestBody @Validated EditClassifyHomeDto param, BindingResult result){
        ...
        ClassifyHome bean = new ClassifyHome();
        BeanUtil.copyProperties(param, bean);
        ClassifyHome repo = service.update(bean);
        return Result.ok();
    }
    ...
    String detail(@PathVariable("id") Long id, Model model){
        model.addAttribute("item", service.findById(id));
        return "/admin/classifyhome/detail";
    }
