...
import com.dk.data.dto.AddProjectsHomeDto;
import com.dk.data.dto.EditProjectsHomeDto;
 ...
 */
@Controller
@RequestMapping(value = "/admin/projectshome")
public class ProjectsHomeController {

    private static final Logger logger = LoggerFactory.getLogger(ProjectsHomeController.class);

    @Autowired
    private ProjectsHomeService service;

    @GetMapping("v_list")
    String vlist(Model model){
        return "/admin/projectshome/list";
    }

    @PostMapping("list")
    @ResponseBody
    PageVo<ProjectsHome> list(@RequestBody SearchProjectsHomeDto condition){
        logger.info("搜索轮播图，参数：\n {}", condition.toString());
        IPage<ProjectsHome> page = service.findAll(condition);
        return Result.page(page);
    }

    @GetMapping("v_add")
    String add(){
        return "/admin/projectshome/add";
    }

    @PostMapping("add")
    @ResponseBody
    Vo add(@RequestBody @Validated AddProjectsHomeDto param, BindingResult result){
        ...
        ProjectsHome bean = new ProjectsHome();
        BeanUtil.copyProperties(param, bean);
        ProjectsHome repo = service.add(bean);
        return Result.ok();
    }
    ...
    String update(@PathVariable("id") Long id, Model model){
        ...
        return "/admin/projectshome/edit";
    }

    @PostMapping("update")
    @ResponseBody
    Vo update(@RequestBody @Validated EditProjectsHomeDto param, BindingResult result){
        ...
        ProjectsHome bean = new ProjectsHome();
        BeanUtil.copyProperties(param, bean);
        ProjectsHome repo = service.update(bean);
        return Result.ok();
    }
    ...
    String detail(@PathVariable("id") Long id, Model model){
        model.addAttribute("item", service.findById(id));
        return "/admin/projectshome/detail";
    }
