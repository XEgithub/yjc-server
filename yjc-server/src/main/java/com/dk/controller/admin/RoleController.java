...
import com.dk.data.dto.AddRoleDto;
import com.dk.data.dto.EditRoleDto;
...
public class RoleController {
    ...
    @PostMapping("list")
    @ResponseBody
    PageVo<Role> list(@RequestBody SearchRoleDto condition){
        logger.info("搜索角色，参数：\n {}", condition.toString());
        IPage<Role> page = service.findAll(condition);
    ...
    @PostMapping("update")
    @ResponseBody
    Vo update(@RequestBody @Validated EditRoleDto param, BindingResult result){
        logger.info("更新角色，参数：\n {}", param.toString());
        if (result.hasErrors()) {
            FieldError fieldError = result.getFieldError();
            String errorMsg = fieldError != null ? fieldError.getDefaultMessage() : "参数异常";
            logger.error("更新角色:{}", errorMsg);
            return Result.paramError(errorMsg);
        }
        Role bean = new Role();
        BeanUtil.copyProperties(param, bean);
        Role repo = service.update(bean);
        return Result.ok();
    }
    ...
    Vo delete(@RequestBody List<Long> ids){
        logger.info("批量删除角色，参数：idList = {}", ids.toArray());
