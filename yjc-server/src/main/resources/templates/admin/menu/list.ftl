...
<@head_dtree title="菜单列表"/>

    <@body_lr>
        <@body_lt_tree>

            <@card_header title="菜单"/>
...
<script>
    layui.config({
        ...
        var myTree = dtree.render({
            ...
            toolbarFun: {
                ...
                editTreeLoad: function(treeNode){
                    $.get('/admin/menu/detail/' + treeNode.pid, function (res) {
                        var param = {
                            name: res.data.name,
                            url: res.data.url,
                            icon: res.data.icon,
                            remark: res.data.remark,
                            sortt: res.data.sortt,
                ...
                delTreeNode: function(treeNode){
                    $.get('/admin/menu/delete/' + treeNode.pid, function (res) {
                        if (res.code === 0) {
                            layer.msg('删除成功！');
        ...
        //新增弹窗
        <@table_add_plugin module="menu"/>
        //批量删除
        <@table_batch_delete_plugin module="menu"/>
        //表格搜索
        <@table_search_plugin/>
        //表格行操作按钮
        <@table_operator_plugin module="menu"/>
        //表格列排序
        <@table_sort_plugin/>
        //表格重载
        <@table_reload_plugin/>
