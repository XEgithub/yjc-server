...
<@head_add_edit title="护士预约关系表-新增"/>

    <@body_empty>
        <@form_padding title="新增护士预约关系表">
            <@input label="护士" name="nurse" placeholder="请输入护士" require="false"/>
            <@input label="预约" name="appointment" placeholder="请输入预约" require="false"/>
            <@input label="状态 0:派单中 1:已拒绝 2:已超时 3:用户取消 4:护士取消 5:已完成" name="status" placeholder="请输入状态 0:派单中 1:已拒绝 2:已超时 3:用户取消 4:护士取消 5:已完成" require="false"/>
...
<script>
    layui.use(['element', 'form', 'laydate'], function(){
        ...
        form.on('submit(*)', function(data){
            $.ajax({
                type: "POST",
                url: "/admin/nurseappointment/add",
