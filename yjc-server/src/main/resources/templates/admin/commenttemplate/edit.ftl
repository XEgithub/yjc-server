...
<@head_add_edit title="评论模板-编辑"/>

    <@body_empty>
        <@form_padding title="编辑评论模板">
            ...
            <@input label="删除" name="delete" value="${(item.delete)!''}" placeholder="请输入删除" require="false"/>
...
<script>
    layui.use(['element', 'form', 'laydate'], function(){
        ...
        form.on('submit(*)', function(data){
            $.ajax({
                type: "POST",
                url: "/admin/commenttemplate/update",
